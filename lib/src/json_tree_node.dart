  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: isHighlighted ? highlightColor.withOpacity(0.3) : null,
      ),
      padding: padding,
      child: Row(
        ...
        children: [
          InkWell(
            onTap: (type == JsonNodeType.object || type == JsonNodeType.array) ? onTap : null,
  ...
  Widget _buildExpandButton() {
    if (type == JsonNodeType.object || type == JsonNodeType.array) {
      return SizedBox(
        width: 20,
        child: isExpanded
            ? (collapseIcon ?? Icon(Icons.keyboard_arrow_down, size: 18, color: Colors.grey[600]))
            : (expandIcon ?? Icon(Icons.keyboard_arrow_right, size: 18, color: Colors.grey[600])),
      );
    }
